...
result$intersection_clusters$all_components #the grouping of Gintersection (chains agree on every monited parameters), those groups as a list of chain-name vector
#the reason they are having the same group of chains is because 3pk model has two peripheral compartmetns taht enter the likihood in the same way, the swap gives the same fit
...
arr <- as.array(fit) #iterations chains parameters
...
d1 <- dens_by_par[["k13"]]$d1
d2 <- dens_by_par[["k13"]]$d2
...
pdf_path <- file.path(docs_dir, "fig_experiment3_k13.pdf")

# Same square size and title size as Experiment 1; L-shaped axes like the a-b scatter.
...
grDevices::dev.off()
file.copy(pdf_path, file.path(out_dir, "fig_experiment3_k13.pdf"), overwrite = TRUE)
cat("wrote", pdf_path, "\n")
